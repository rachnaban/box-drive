...
class Upload extends React.Component {
  ...
  processFiles = async (e, files) => {
    ...
    this.setState({files: allFiles.length});
    allFiles.forEach(async file => {
      await this.uploadFile(e, file);
    });
  };
  renderThumbnails =(loaded, total)=>{
    
      return <Thumbnail source={image} key={image} />;
  ...
  render() {
    return (
      <div>
        ...
        {this.state.files.length > 0 ? for(const i =0;i<this.state.files;i++){<Thumbnail/>} :null }
